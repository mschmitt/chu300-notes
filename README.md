...
**ATTENTION: The USB-C charger shipping with the Chuwi book is a non-compliant non-negotiating fixed 12 V power supply and WILL wreck regular USB-C devices, which DID kill two USB gadgets of mine!**
...
* I have no fix and reproducer for the subtle display flicker that sometimes occurs. There *is* a possibility that there's a correlation with use of the included non-compliant power supply, which I trashed.
...
With this workaround in place, the *xe* driver seems to try to take precedece over *i915* sometimes, so it needs to be blocklisted in the kernel command line in */etc/default/limine*:
...
The display was rotated in KDE display settings to match the laptop's orientation. (Never used in tablet or "tent" mode.) Then the plasma display settings were copied to the login screen settings using the GUI.
